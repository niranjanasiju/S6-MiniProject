...
adr_hdata['drug'].node_idx        = torch.arange(N_ADR_DRUGS, dtype=torch.long)
adr_hdata['side_effect'].node_idx = torch.arange(N_ADR_SE,    dtype=torch.long)
adr_hdata['protein'].node_idx     = torch.arange(N_ADR_PROT,  dtype=torch.long)
adr_hdata['pathway'].node_idx     = torch.arange(N_ADR_PATH,  dtype=torch.long)
...
def predict_adr_route(req: ADRRequest):
    ...
    if norm not in adr_drug2idx:
        raise HTTPException(status_code=404,
                            detail=f"Drug '{req.drug_name}' not in ADR database. "
                                   f"Supported: {', '.join(sorted(adr_all_drugs))}")

    predicted = predict_adr(
        drug_name=norm, adr_model=adr_model, adr_hdata=adr_hdata,
        adr_drug2idx=adr_drug2idx, adr_idx2se=adr_idx2se,
        SIDE_EFFECT_CATEGORIES=SIDE_EFFECT_CATEGORIES,
        N_ADR_SE=N_ADR_SE, threshold=req.threshold,
    )
    ...
    ai_explanation = get_adr_ai_explanation(
        drug_name=norm, side_effects=predicted,
        targets=targets, pathways=pathways,
    )

    return {'drug': norm, 'threshold': req.threshold,
            'targets': targets, 'pathways': pathways,
            'side_effects': predicted, 'ai_explanation': ai_explanation}
